...
class OutputPDF {
    ...
    public function __construct($idOutput,$createFile = null) {
        $output = new OutputRepository();
        $output->setOptions($output->getById($idOutput));
        $output->setId($idOutput);
        ...
        $pdf->Cell(190,7, "SALIDA #: ".$idOutput, '0', 1, 'C');     
        $pdf->Cell(190,7, "SUCURSAL: ".$sucursal->getName(), '0', 1, 'C');     
       
        $pdf->SetFont('Arial','B','8');         
        $pdf->cell(95,6,'INFORMACION DE SALIDA:','0',1,'L'); 
        ...
        $pdf->Cell(25, 4, "Entregado por:", '0', 0, 'R');
        $pdf->Cell(75, 4, $output->getUserName(), '0', 1, 'L');
        ...
        $pdf->SetFont('Arial','','8');
        $detalles = $output->getOutputDetailsSaved($idOutput);
        $cantidadItems = 0;
        ...
        if($createFile){
            ...
            $pdf->Output(PATH_TEMP_DOCS."ouputs/PURCHASE-".$idOutput.".pdf","F");
            $this->pathfFileCreated = PATH_TEMP_DOCS."ouputs/PURCHASE-".$idOutput.".pdf";
        }else{
            $pdf->Output();
        }
    }

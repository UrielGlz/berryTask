...
class SizeRepository extends EntityRepository {
    ...
    public function getById($id, $table = null,$selectAux = null) {
        $query = "SELECT *,fxGetStatusName(status,'Size')as status_name FROM $this->table WHERE id = '$id'";
    ...
    public function isUsedInRecord($id, array $buscarEn = null,$andWhere = null) {
        $query = "SELECT id FROM products WHERE size = '$id'";
        $result = $this->query($query);
        if($result->num_rows > 0){
            return true;
        }
    ...
    public function getListSelectSizes(){        
     ...
     public function getListSizes($options = null){          
        $query = "SELECT *,"
                . "fxGetStatusName(status,'Size')as status_name "
                . "FROM $this->table ";
        $result = $this->query($query);
    ...
    public function getListStatus(){
        $query = "SELECT * FROM status_code WHERE operation = 'Size'";
        $result = $this->query($query);

...
class OutputRepository extends EntityRepository {
    ...
    public function save(array $data, $table = null) {             
        ...
        $idOutput = $this->getInsertId();
        $this->setLastInsertId($idOutput);//Para utilizarlo en el Controller action insert
        
        if($rs){
            $purchaseDetailsTemp = new OutputDetailsTempRepository();

            if($purchaseDetailsTemp->saveDetalles($idOutput,$this->getTokenForm())){   
                $this->commit();
    ...
    public function delete($id, $table = null) {
        ...
        if($rs){
            $detallesAfectados = $this->getOutputDetailsSaved($id);
            if(!$this->addInventoryFromOutputDetalles($detallesAfectados)){
                $this->rollback();
                return null;
            }    
    ...
    public function update($id, $data, $table = null) {                
        ...
        if($result){
            $repository = new OutputDetailsTempRepository();
    ...
    public function getById($id, $table = null,$selectAux = null) {
        $select = "SELECT *,"
                ...
                . "fxGetStatusName(status,'Output')as statusName, "
    ...
    public function crearTablaDetallesForUser(){
        $login = new Login();        
        $query = "CREATE TABLE IF NOT EXISTS output_details_".$login->getId()." 
                 (  
                    `token_form` char(50) NOT NULL,
                    `id` int(11) NOT NULL AUTO_INCREMENT,
                    `id_detail` int(11) NULL,
                    `id_output` int(11) NULL,
    ...
    public function getOutputDetails($token_form){
        $login = new Login();
        $query = "SELECT v.*,
                  ...
                  FROM output_details_".$login->getId()." v LEFT JOIN products p
    ...
    public function getOutputDetailSaved($id){
        $query = "SELECT c.*,
                    p.code as code
                    FROM output_details c LEFT JOIN products p ON c.id_product = p.id
                    WHERE c.id = '$id'";
        $result = $this->query($query);
    ...
    public function getOutputDetailsSaved($id){
        $query = "SELECT c.*,
                    p.code as code,
                    p.description,
    ...
    public function setOutputDetailsById($idOutput,$tokenForm){
        ...
        return $repository->setOutputDetailsById($idOutput,$tokenForm);
    }
    ...
    public function getListOutput(){ 
        ...
        $query = "SELECT c.*,
                ...
                fxGetStatusName(c.`status`,'Output')as statusName
                FROM outputs c
                WHERE  1=1 
                $store_id " 
              . "GROUP BY c.id "
   ...
   public function addInventoryFromOutputDetalles($detallesAfectados){

<?php
class ReturnDetailsTempRepository extends EntityRepository {

    private $table = 'return_details_';
    ...
    public function save(array $data, $table = null) {
        ...
        unset($data['id_detail'],$data['id_return']);
    ...
    public function saveDetalles($idReturn,$tokenForm){
    ...
    public function updateDetalles($idReturn,$tokenForm){                
        $query = "SELECT id,id_product,quantity,location FROM return_details WHERE id_return = '$idReturn'";
        $result = $this->query($query);
        $detallesOrigin = null;    
        if($result->num_rows > 0){
        ...
        if($result->num_rows > 0){
            ...
            foreach($result as $data){
                ...
                if(!$idDetalle){
                    ...
                    if(parent::update($idDetalle,$data, 'return_details')){
        ...
        if($detallesOrigin){
            foreach ($detallesOrigin as $detalle){
                if(!in_array($detalle['id'], $idsDetalles)){
                    if(parent::delete($detalle['id'], 'return_details')){
